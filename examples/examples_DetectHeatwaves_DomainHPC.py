...
hwYears = [year for year in range(1979,2000+1)]
...
eventDictKeys = ['index_start', 'index_end', 'date_start', 'date_end',
                 'max_temp', 'date_peak', 'index_peak', 'duration', 
...
if rank == 0:
    ...
    for curr_day in range(NoI):
        print(f'handling window-percentile for day: {curr_day} (365)')
...
for hwYear in hwYears:
    # defining variable for other ranks
    chunk        = None
    eventChunk   = None
    dailyMean_hw = None
    dailyTime_hw = None
    ...
    if rank == 0:
        ...
        lst = [ item for item in event_labels[event_label_counts>=minDuration] ]
        n = len(lst) // (size-1)
        chunk = chunks(lst, n)

    print('RANK {}: waiting until root is ready BEFORE LOOP'.format(rank))
    comm.Barrier()

    eventChunk   = comm.scatter(chunk, root=0)
    dailyMean_hw = comm.bcast(dailyMean_hw, root=0)
    dailyTime_hw = comm.bcast(dailyTime_hw, root=0)
    clima_90p    = comm.bcast(clima_90p, root=0)
    clima_smooth = comm.bcast(clima_smooth, root=0)
    ...
    #     ###############################################################################
    tmp_DictKeys = [f'{hwYear}_{ev}' for ev in eventChunk]
    # tmp_DictKeys = [f'{hwYear}_{ev}' for ev in event_labels[event_label_counts>=minDuration]]
    tmp_HWevents = dict.fromkeys(tmp_DictKeys)
    print(f'needed HWevents file were not found: calculate')
    for ev in eventChunk:
    # for ev in event_labels[event_label_counts>=minDuration]:
        ...
        tmp_HWevents[tmp_dict_key] = dict.fromkeys(eventDictKeys)
    ...
    print('RANK {}: waiting until root is ready AFTER LOOP'.format(rank))
    comm.Barrier()
    print('COLLECTING')
    tmp_HWevents = comm.gather(tmp_HWevents, root=0)
    for items in tmp_HWevents:
        # ADD FOR FINAL USE
        # merging two dicts:
        HWevents = {**HWevents, **items}
